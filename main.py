...
Alerts live in ids_alerts.db (SQLite, WAL mode -- see db.py). This process is
the only one that ever writes to that file:

  - ids.py running as the LOCAL sensor (same box) forwards alerts here over
    HTTP, same as a remote sensor would -- see alert.py's IDS_FORWARD_URL.
  - ids.py running on a remote sensor (e.g. a Pi) forwards here the same way,
    just pointed at whatever host:port this process is bound to instead of
    loopback (see the run instructions below).
  - Alerts generated by THIS process itself (e.g. clicking "Unblock" on the
    dashboard, which calls ips.unblock_ip() in-process) go straight into the
    db via alert.py's local-sink hook, registered at startup below -- no
    HTTP loopback, so a click can't block the request handler on itself.
...
Run this from the same directory as ids_alerts.db (i.e. ~/ids):

    pip install -r requirements.txt

    # loopback only -- nothing but this box (or something you've tunneled,
    # e.g. Tor) can reach it:
    uvicorn main:app --host 127.0.0.1 --port 8000

    # reachable by a remote sensor (e.g. a Pi over Tailscale) -- bind to
    # that specific interface IP, NOT 0.0.0.0, so it isn't exposed on every
    # NIC the box happens to have:
    uvicorn main:app --host <this-box-tailscale-ip> --port 8000

    # if you're also running ids.py locally on THIS box in that second mode,
    # point its IDS_FORWARD_URL at <this-box-tailscale-ip>:8000/api/ingest,
    # not 127.0.0.1 -- once uvicorn is bound to a specific interface, it
    # stops listening on loopback.

Then open http://<host>:8000 in a browser. Run this alongside ids.py -- it
never touches sniffing/detection logic, only the alerts they produce.
"""
...
# SENSOR_NAME. A remote sensor (e.g. a Pi) arrives with sensor="pi-sensor"
# (or whatever IDS_SENSOR_NAME is set to there) and won't match -- unblocking
# those needs a remote control call, not built yet (see api_unblock).
...
# rebuilt at startup by replaying IP_BLOCKED/IP_UNBLOCKED rows from the db,
# then kept in sync incrementally as new events arrive (see ingest_alert)
blocked_state = {}
...
def update_block_state(data):
    ...
    flow through, whether from startup replay or a live ingest."""
    rule = data.get("rule")
    ip = data.get("src")
    if not ip:
        return
...
async def api_unblock(request: Request):
    ...
    if entry.get("sensor") != LOCAL_SENSOR_NAME:
        # TODO: once a remote sensor runs its own control endpoint, dispatch
        # this unblock request to that sensor over the network instead of
        # erroring. Relevant now that blocking happens on the Pi, not Kali --
        # most blocked IPs will hit this branch until that's built.
        return JSONResponse({
            "error": (
    ...
    unblock_ip(ip)  # fires an IP_UNBLOCKED alert via the local sink -> ingest_alert
