...
the only one that ever writes to that file:
  ...
  - Alerts generated by THIS process itself go straight into the db via
    alert.py's local-sink hook, registered at startup below -- no HTTP
    loopback needed.

This dashboard is read-only with respect to blocking: it can show which
IPs are blocked, but it does not unblock them. Unblocking only happens
automatically (ids.py unblocks everything it blocked when it shuts down)
or manually via `python3 ips.py --unblock <ip>` on the sensor that holds
the block.
